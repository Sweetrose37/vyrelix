...
export const store={
  ...
  saveProject(input){
    const now=iso(),project={id:input.id||id(),studio:input.studio,builderType:input.builderType||"Builder",title:(input.title||"Untitled Nyvera Project").trim(),createdAt:input.createdAt||now,updatedAt:now,favorite:Boolean(input.favorite),status:input.status||"completed",formData:input.formData||{},generatedPrompt:input.generatedPrompt||"",negativePrompt:input.negativePrompt||"",mockupPrompt:input.mockupPrompt||"",selectionSummary:input.selectionSummary||{},templateId:input.templateId||null,mockupEnabled:Boolean(input.mockupEnabled),mockupStatus:input.mockupStatus||"",mockupFormData:input.mockupFormData||null,mockupNegativePrompt:input.mockupNegativePrompt||"",mockupCreatedAt:input.mockupCreatedAt||null,mockupUpdatedAt:input.mockupUpdatedAt||null,mockupFavorite:Boolean(input.mockupFavorite),mockups:Array.isArray(input.mockups)?input.mockups:[],productEnabled:Boolean(input.productEnabled),productStatus:input.productStatus||"",productType:input.productType||"",productCategory:input.productCategory||"",productFormData:input.productFormData||null,productPrompt:input.productPrompt||"",productNegativePrompt:input.productNegativePrompt||"",productCreatedAt:input.productCreatedAt||null,productUpdatedAt:input.productUpdatedAt||null,productFavorite:Boolean(input.productFavorite),linkedProductIds:Array.isArray(input.linkedProductIds)?input.linkedProductIds:[],products:Array.isArray(input.products)?input.products:[],appVersion:APP_VERSION};
